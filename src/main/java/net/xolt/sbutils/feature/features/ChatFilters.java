...
public class ChatFilters extends Feature<ModConfig> {
    ...
    private int onSetRegexCommand(int index, String newRegex) {
        List<CustomFilter> filters = ModConfig.HANDLER.getConfig().chatFilters.customFilters;
    ...
    public void recompileCustomRegex() {
        ...
        for (CustomFilter filter : ModConfig.HANDLER.getConfig().chatFilters.customFilters) {
            if (!filter.enabled)
                continue;
