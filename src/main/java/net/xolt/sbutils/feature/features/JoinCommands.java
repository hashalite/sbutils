...
public class JoinCommands extends Feature<ModConfig> {
    ...
    private static int onSetCommandCommand(int index, String newCommand) {
        List<ModConfig.JoinCommandsConfig.JoinCommandsEntry> joinCommands = ModConfig.HANDLER.getConfig().joinCommands.commands;
    ...
    public void tick() {
        if (!ModConfig.HANDLER.getConfig().joinCommands.enabled || !SbUtils.SERVER_DETECTOR.isOnSkyblock() || commandQueue.isEmpty())
            return;

        int delay = (int)((joinedAt > lastCommandSentAt ? ModConfig.HANDLER.getConfig().joinCommands.initialDelay : ModConfig.HANDLER.getConfig().joinCommands.delay) * 1000.0);
    ...
    public void onJoinGame() {
        if (!ModConfig.HANDLER.getConfig().joinCommands.enabled || ModConfig.HANDLER.getConfig().joinCommands.commands.isEmpty() || MC.player == null)
            return;
    ...
    private static List<String> getJoinCommands(Player player) {
        ...
        return ModConfig.HANDLER.getConfig().joinCommands.commands.stream().filter((command) -> {
