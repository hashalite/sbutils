...
public class AutoReply extends Feature<ModConfig> {
    ...
    public void tick() {
        ...
        if (ModConfig.HANDLER.getConfig().autoReply.statusHud && MC.player != null)
    ...
    public void processMessage(Component message) {
        ...
        if (incomingMsg.matches()) {
            ...
            if (System.currentTimeMillis() - playersLastResponse.getOrDefault(player, 0L) > ModConfig.HANDLER.getConfig().autoReply.playerCooldown * 1000)
    ...
    private void sendMessage() {
        ...
        SbUtils.COMMAND_SENDER.sendCommand("msg " + player + " " + ModConfig.HANDLER.getConfig().autoReply.response);
