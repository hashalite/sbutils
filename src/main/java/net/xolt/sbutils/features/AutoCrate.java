...
public class AutoCrate {
    ...
    public static void registerCommand(CommandDispatcher<FabricClientCommandSource> dispatcher) {
        final LiteralCommandNode<FabricClientCommandSource> autoCrateNode = dispatcher.register(ClientCommandManager.literal("autocrate")
                ...
                .then(ClientCommandManager.literal("mode")
                        ...
                        .then(ClientCommandManager.argument("mode", ModConfig.CrateMode.CrateModeArgumentType.crateMode())
                                .executes(context -> {
                                    ModConfig.INSTANCE.getConfig().crateMode = ModConfig.CrateMode.CrateModeArgumentType.getCrateMode(context, "mode");
                                    ModConfig.INSTANCE.save();
                                    Messenger.printChangedSetting("text.sbutils.config.option.crateMode", ModConfig.INSTANCE.getConfig().crateMode);
                                    return Command.SINGLE_SUCCESS;
                                })))
    ...
    private static Pattern getKeyFilter() {
        switch(ModConfig.INSTANCE.getConfig().crateMode) {
            case COMMON:
