...
public class AutoMine {
    ...
    public static void registerCommand(CommandDispatcher<FabricClientCommandSource> dispatcher) {
        ...
        final LiteralCommandNode<FabricClientCommandSource> autoMineNode = dispatcher.register(
                CommandUtils.toggle(COMMAND, "autoMine", () -> ModConfig.INSTANCE.autoMine.autoMine, (value) -> {ModConfig.INSTANCE.autoMine.autoMine = value; if (!value) reset();})
                    ...
                    .then(CommandUtils.bool("switch", "autoMine.autoSwitch", () -> ModConfig.INSTANCE.autoMine.autoSwitch, (value) -> ModConfig.INSTANCE.autoMine.autoSwitch = value))
                    .then(CommandUtils.integer("durability", "durability", "autoMine.switchDurability", () -> ModConfig.INSTANCE.autoMine.switchDurability, (value) -> ModConfig.INSTANCE.autoMine.switchDurability = value))
        );
    ...
    private static int onTimerCommand(int time) {
        timer = time;
        ModConfig.INSTANCE.autoMine.autoMine = true;
    ...
    public static void tick() {
        if (!ModConfig.INSTANCE.autoMine.autoMine || MC.player == null) {
            return;
        }
        ...
        if (timer > 0) {
            ...
            if (timer == 0) {
                ...
                Messenger.printChangedSetting("text.autoconfig.sbutils.option.autoMine.autoMine", false);
                return;
            }
        }
        ...
        if (ModConfig.INSTANCE.autoMine.autoSwitch && !AutoFix.fixing() && holding.getItem() instanceof PickaxeItem && holding.getMaxDamage() - holding.getDamage() <= minDurability) {
            int newPickaxeSlot = findNewPickaxe();
            if (newPickaxeSlot != -1) {
                InvUtils.swapToHotbar(newPickaxeSlot, MC.player.getInventory().selectedSlot);
            } else if (!ModConfig.INSTANCE.autoFix.autoFix) {
                ModConfig.INSTANCE.autoMine.autoMine = false;
                ModConfig.HOLDER.save();
    ...
    private static int findNewPickaxe() {
        ...
        int minDurability = Math.max(ModConfig.INSTANCE.autoMine.switchDurability, ModConfig.INSTANCE.toolSaver.toolSaver ? ModConfig.INSTANCE.toolSaver.toolSaverDurability : 0);
    ...
    private static int getMinDurability() {
        return Math.max(ModConfig.INSTANCE.autoMine.autoSwitch ? ModConfig.INSTANCE.autoMine.switchDurability : -1, ModConfig.INSTANCE.toolSaver.toolSaver ? ModConfig.INSTANCE.toolSaver.toolSaverDurability : -1);
    }
